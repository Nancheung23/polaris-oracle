...
#[test]
fn test_update_operator_success() {
    ...
    // ix for update operator
    let new_operator = Keypair::new();
    let ix = test_context
        .svm
        .program()
        .accounts(anchor::accounts::UpdateOperator {
            authority: test_context.admin.pubkey(),
            platform_pda,
        })
        .args(anchor::instruction::UpdateOperator {
    ...
    assert_eq!(platform_state.operator, new_operator.pubkey());
}

#[test]
fn test_update_operator_failed() {
    // get context from mod.rs
    let mut test_context = setup();
    // check mod.rs
    let platform_pda = platform_pda(&test_context.admin.pubkey());
    ...
    let ix = test_context
        .svm
        .program()
        // give wrong authority
        .accounts(anchor::accounts::UpdateOperator {
            authority: test_context.user.pubkey(),
            platform_pda,
        })
        .args(anchor::instruction::UpdateOperator {
            new_operator: new_operator.pubkey(),
    ...
    assert!(
        ...
        "expected UnauthorizedAdmin"
    );
    // assert error code
    let expected_code =
        PolarisError::UnauthorizedAdmin as u32 + anchor_lang::error::ERROR_CODE_OFFSET;
