...
fn test_consume_ticket_failed() {
    ...
    // assert
    assert!(result.is_ok());
    // ix for consume ticket. make tickets = 0
    let ix = test_context
        .svm
        .program()
        .accounts(anchor::accounts::ConsumeTicket {
    ...
    let expected_code =
        PolarisError::InsufficientTicket as u32 + anchor_lang::error::ERROR_CODE_OFFSET;
